...
from shared.models.inventory import Product, InventoryItem, Warehouse, ProductStatus, InventoryStatus
...
class InventoryService:
    ...
    def create_product(self, product_data: Dict[str, Any]) -> Product:
        ...
        try:
            ...
            product = Product(
                ...
                unit_price=Decimal(str(product_data['unit_price'])) if product_data.get('unit_price') else None,
    ...
    def update_product(self, sku: str, update_data: Dict[str, Any]) -> Optional[Product]:
        ...
        try:
            ...
            allowed_fields = [
                ...
                'weight', 'dimensions', 'barcode', 'qr_code', 'status',
                'reorder_point', 'reorder_quantity', 'supplier_id',
                'supplier_sku', 'tags', 'images'
            ]
    ...
    def list_products(self, page: int = 1, limit: int = 20, category: str = None,
                     status: str = None, search: str = None) -> List[Product]:
                ...
                try:
                    ...
                    if search:
                        ...
                        if (search_lower in product.name.lower() or
                            ...
                            search_lower in product.sku.lower()):
    ...
    def create_warehouse(self, warehouse_data: Dict[str, Any]) -> Warehouse:
        ...
        try:
            ...
            if existing_warehouse:
                raise ValueError(f"Warehouse with ID {warehouse_data['warehouse_id']} already exists")
            ...
            # Save to Firestore
            self.warehouses_collection.document(warehouse.warehouse_id).set(warehouse.to_dict())
    ...
    def get_inventory_by_warehouse_sku(self, warehouse_id: str, sku: str) -> Optional[InventoryItem]:
    ...
    def create_or_update_inventory(self, warehouse_id: str, sku: str, 
                                 quantity: int, cost_per_unit: Decimal = None,
                                 location: str = None) -> InventoryItem:
        ...
        try:
            ...
            if existing_item:
                ...
                existing_item.quantity_available = max(0, quantity - existing_item.quantity_reserved)
                if cost_per_unit:
                ...
                # Save to Firestore
                self.inventory_collection.document(inventory_id).update(existing_item.to_dict())
    ...
    def list_inventory(self, warehouse_id: str = None, sku: str = None,
                      status: str = None, low_stock: bool = None,
                      page: int = 1, limit: int = 50) -> List[InventoryItem]:
        ...
        try:
            ...
            # Apply pagination
            offset = (page - 1) * limit
            query = query.order_by('updated_at', direction='DESCENDING').offset(offset).limit(limit)
    ...
    def adjust_inventory(self, inventory_id: str, new_quantity: int, 
                        reason: str, user_id: str) -> Optional[InventoryItem]:
        ...
        try:
            ...
            logger.info(f"Inventory adjusted: {inventory_id} from {old_quantity} to {new_quantity}")
            return item
    ...
    def reserve_inventory(self, inventory_id: str, quantity: int,
                         order_id: str = None, user_id: str = None) -> bool:
    ...
    def release_inventory(self, inventory_id: str, quantity: int, user_id: str = None) -> bool:
    ...
    def find_by_barcode(self, barcode: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for doc in product_query.stream():
                ...
                inventory_query = self.inventory_collection.where('sku', '==', product.sku)
                inventory_items = []
                ...
                for inv_doc in inventory_query.stream():
                    try:
                        ...
                        logger.warning(f"Failed to parse inventory document {inv_doc.id}: {e}")
    ...
    def transfer_inventory(self, from_warehouse_id: str, to_warehouse_id: str,
                          sku: str, quantity: int, notes: str = None, user_id: str = None) -> bool:
        ...
        try:
            ...
            source_item.quantity_available = max(0, source_item.quantity_on_hand - source_item.quantity_reserved)
            source_item.updated_at = datetime.utcnow()
            
            # Update or create destination inventory
            if dest_item:
                dest_item.quantity_on_hand += quantity
                dest_item.quantity_available = max(0, dest_item.quantity_on_hand - dest_item.quantity_reserved)
            ...
            # Save both items
            self.inventory_collection.document(source_item.inventory_id).update(source_item.to_dict())
            self.inventory_collection.document(dest_item.inventory_id).set(dest_item.to_dict())
            ...
            self._record_movement(
                ...
                quantity=-quantity,
                reason=f"Transfer to {to_warehouse_id}: {notes}" if notes else f"Transfer to {to_warehouse_id}",
                user_id=user_id,
            ...
            self._record_movement(
                ...
                quantity=quantity,
                reason=f"Transfer from {from_warehouse_id}: {notes}" if notes else f"Transfer from {from_warehouse_id}",
                user_id=user_id,
                reference_id=transfer_id
            )
            
            logger.info(f"Inventory transferred: {quantity} of {sku} from {from_warehouse_id} to {to_warehouse_id}")
            return True
    ...
    def _record_movement(self, warehouse_id: str, sku: str, movement_type: str,
                        quantity: int, reason: str = None, user_id: str = None,
