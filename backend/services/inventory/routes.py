...
from flask_jwt_extended import jwt_required  # noqa: F401
...
def list_products():
    ...
    try:
        ...
        products = inventory_service.list_products(
            page=page, limit=limit, category=category, status=status, search=search
        )
...
def adjust_inventory(inventory_id):
    ...
    try:
        ...
        updated_item = inventory_service.adjust_inventory(
            inventory_id, new_quantity, reason, request.user_id
        )
...
def reserve_inventory(inventory_id):
    ...
    try:
        ...
        success = inventory_service.reserve_inventory(
            inventory_id, quantity, order_id, request.user_id
        )
...
def release_inventory(inventory_id):
    ...
    try:
        ...
        success = inventory_service.release_inventory(
            inventory_id, quantity, request.user_id
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "low_stock_items": [
                        {"inventory_item": item.to_dict(), "reorder_needed": True}
...
def transfer_inventory():
    ...
    try:
        ...
        success = inventory_service.transfer_inventory(
            from_warehouse_id, to_warehouse_id, sku, quantity, notes, request.user_id
        )
        ...
        if not success:
            return (
                jsonify(
                    {
                        "error": "Transfer failed - insufficient inventory or invalid warehouses"
                    }
                ),
                400,
            )
