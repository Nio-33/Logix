...
class AuthService:
    ...
    def list_users(self, page: int = 1, limit: int = 20, role_filter: str = None) -> List[User]:
        """
    ...
    def search_users(self, search_term: str, limit: int = 10) -> List[User]:
        ...
        try:
            ...
            email_queries = [
                ...
                self.users_collection.where('email', '>=', search_term).where('email', '<=', search_term + '\uf8ff')
            ]
    ...
    def get_warehouse_staff(self, warehouse_id: str) -> List[User]:
        ...
        try:
            query = (self.users_collection
                    .where('role', '==', UserRole.WAREHOUSE_STAFF.value)
