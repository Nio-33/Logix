...
class RouteService:
    ...
    def create_route(self, route_data: Dict[str, Any]) -> Route:
        ...
        try:
            ...
            optimized_route = self._optimize_route_with_ai(stops_data, route_data.get('constraints'))
            
            # Create route stops
            stops = []
            for i, stop_data in enumerate(optimized_route.get('optimized_sequence', stops_data)):
                stop = RouteStop(
            ...
            route = Route(
                ...
                status='planned',
                planned_start_time=route_data.get('planned_start_time', datetime.utcnow()),
    ...
    def update_route_status(self, route_id: str, new_status: str, user_id: str) -> Optional[Route]:
        """Update route status"""
        try:
            ...
            if new_status == 'active' and not route.actual_start_time:
                ...
                route.actual_duration = int((route.actual_end_time - route.actual_start_time).total_seconds() / 60)
            
            # Save to Firestore
            self.routes_collection.document(route_id).update(route.to_dict())
            
            logger.info(f"Route {route_id} status updated from {old_status} to {new_status}")
            return route
    ...
    def update_stop_status(self, route_id: str, stop_id: str, new_status: str, 
                          delivery_proof: Dict[str, Any] = None) -> Optional[Route]:
            ...
            for stop in route.stops:
                if stop.stop_id == stop_id:
                    ...
                    if new_status == 'arrived':
                        ...
                        stop.notes = delivery_proof.get('failure_reason', '') if delivery_proof else ''
    ...
    def get_driver_routes(self, driver_id: str, status: str = None, 
                         date: datetime = None) -> List[Route]:
        ...
        try:
            ...
            if date:
                ...
                query = query.where('planned_start_time', '>=', start_of_day.timestamp())
    ...
    def optimize_existing_route(self, route_id: str, constraints: Dict[str, Any] = None) -> Optional[Route]:
        ...
        try:
            ...
            route.total_distance = optimized_route.get('total_distance_km', route.total_distance)
            route.estimated_duration = optimized_route.get('total_time_minutes', route.estimated_duration)
            route.fuel_estimate = optimized_route.get('fuel_estimate_liters', route.fuel_estimate)
            ...
            if optimized_sequence:
                ...
                for i, stop_id in enumerate(optimized_sequence):
                    if stop_id in stop_map:
                        ...
                        stop.sequence = i + 1
                        stop.planned_arrival = self._calculate_planned_arrival(i, optimized_route)
        ...
        try:
            ...
            if route.actual_duration and route.estimated_duration:
                analytics['planned_vs_actual_time'] = {
                    ...
                    'variance_minutes': route.actual_duration - route.estimated_duration,
                    'variance_percentage': ((route.actual_duration - route.estimated_duration) / route.estimated_duration) * 100
                }
            
            # Stop analysis
            completed_stops = len([stop for stop in route.stops if stop.status == 'completed'])
            failed_stops = len([stop for stop in route.stops if stop.status == 'failed'])
            total_stops = len(route.stops)
            
            analytics['stops_completed'] = completed_stops
            analytics['stops_failed'] = failed_stops
            analytics['delivery_success_rate'] = (completed_stops / total_stops) * 100 if total_stops > 0 else 0
            
            # On-time analysis
            on_time_stops = 0
            for stop in route.stops:
                if stop.actual_arrival and stop.planned_arrival:
                    if stop.actual_arrival <= stop.planned_arrival + timedelta(minutes=15):  # 15 min tolerance
                        on_time_stops += 1
            
            analytics['on_time_percentage'] = (on_time_stops / total_stops) * 100 if total_stops > 0 else 0
    ...
    def _optimize_route_with_ai(self, stops_data: List[Dict[str, Any]], 
        ...
        try:
            ...
            return {
                'optimized_sequence': [stop.get('id', i) for i, stop in enumerate(stops_data)],
                'total_distance_km': 50,  # Default estimate
                'total_time_minutes': len(stops_data) * 30,  # 30 min per stop
                'fuel_estimate_liters': 10,
                'recommendations': ['AI optimization unavailable, using default routing']
            }
    
    def _calculate_planned_arrival(self, stop_index: int, optimized_route: Dict[str, Any]) -> datetime:
        ...
        travel_time_per_stop = optimized_route.get('total_time_minutes', 0) / max(1, len(optimized_route.get('optimized_sequence', [])))
